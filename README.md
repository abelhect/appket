...
Dev Env Dependencies:
  ...
  1. redux-devtools (for persisting states)
  2. gh-pages (for deploying in github)
  3. enzyme enzyme-adapter-react-16 react-test-renderer (for
  test driven development)

* Holding off on the below for now because I removed Jest
  4. Also added the jest coverage options in the JSON file to ignore the index.js file that only has libraries that come from FACEBOOK and airbnb

NOTE: don't install latest jest because it breaks the test environment.


Prod Env Dependencies:
  npm install --save
  1. redux react-redux (For persisting state)
  2. react-bootstrap (For beautifying your UI)
  3. prop-types (For test driven development to fix a deprecated issue)
  4. sfcookies (for browser cookies)
  5. react-router-dom (in order to do proper routing in browser)
  6. firebase (to hook to firebase)




Below is default create react app ReadMe
--------------------------
